...
def plot_images(
    ...
    for i in range(bs):
        ...
        img = images[i].transpose(1, 2, 0)  # CHW to HWC (C, H, W) -> (H, W, C)
        # 处理多光谱图像：RGB对应通道4、2、1
        if img.shape[2] > 3:
            img = img[:, :, [4, 2, 1]]  # RGB = 通道4、2、1
            # 处理标准化后的数值范围
            img_min, img_max = img.min(), img.max()
            if img_max > img_min:  # 避免除零
                img = (img - img_min) / (img_max - img_min) * 255  # 归一化到0-255
            img = np.clip(img, 0, 255).astype(np.uint8)  # 确保数据类型和范围
        elif img.shape[2] == 1:
            img = np.repeat(img, 3, axis=2)  # 灰度图转RGB
        # 确保图像尺寸匹配
        if img.shape[:2] != (h, w):
...
def plot_results(file="path/to/results.csv", dir="", on_plot=None):
    """
    Plot training results from a results CSV file for detection and OBB tasks.
    Plots are saved as 'results.png' in the directory where the CSV is located.
