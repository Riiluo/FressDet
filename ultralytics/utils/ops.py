...
def xywh2xyxy(x):
    ...
    """
    assert x.shape[-1] == 4, f"input shape last dimension expected 4 but input shape is {x.shape}"
    y = torch.empty_like(x) if isinstance(x, torch.Tensor) else np.empty_like(x)  # faster than clone/copy  # 2,23142,4
    xy = x[..., :2]  # centers
...
def xywhr2xyxyxyxy(x):
    ...
    ctr = x[..., :2]  # 2,22,2
    w, h, angle = (x[..., i : i + 1] for i in range(2, 5))  # 2,22,1  2,22,1  2,22,1
    cos_value, sin_value = cos(angle), sin(angle)  # 2,22,1
    vec1 = [w / 2 * cos_value, w / 2 * sin_value]
    vec2 = [-h / 2 * sin_value, h / 2 * cos_value]
    vec1 = cat(vec1, -1)  # 2,22,2
    vec2 = cat(vec2, -1)  # 2,22,2
    pt1 = ctr + vec1 + vec2  # 2,22,2
    pt2 = ctr + vec1 - vec2  # 2,22,2
    pt3 = ctr - vec1 - vec2  # 2,22,2
    pt4 = ctr - vec1 + vec2  # 2,22,2
    return stack([pt1, pt2, pt3, pt4], -2)  # 2,22,4,2
...
def segments2boxes(segments):
    ...
    """
    boxes = []
    for s in segments:  # 每个s都是(4,2)的数组
        x, y = s.T  # segment xy
