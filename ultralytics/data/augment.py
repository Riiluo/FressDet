...
class RandomPerspective:
    ...
    def apply_segments(self, segments, M):
        ...
        """
        n, num = segments.shape[:2]  # 3,100
        if n == 0:
            return [], segments

        xy = np.ones((n * num, 3), dtype=segments.dtype)  # 1,1,1 [3]
        segments = segments.reshape(-1, 2)  # (300, 2)
        xy[:, :2] = segments  # (300, 3)
        xy = xy @ M.T  # transform (300,3) @ (3,3) = (300,3)
        xy = xy[:, :2] / xy[:, 2:3]  # (300,2)
        segments = xy.reshape(n, -1, 2)  # (3,100,2)
        bboxes = np.stack([segment2box(xy, self.size[0], self.size[1]) for xy in segments], 0)  # (3,4)
    ...
    def __call__(self, labels):
        ...
        if self.pre_transform and "mosaic_border" not in labels:
            labels = self.pre_transform(labels)  # Letterbox
        labels.pop("ratio_pad", None)  # do not need ratio pad

        img = labels["img"]  # (928,1216,8)
        cls = labels["cls"]  # (3,1)
        instances = labels.pop("instances")  # bbox_areas[22,1] bboxes[22,4]
        # Make sure the coord formats are right
        instances.convert_bbox(format="xyxy")  # (22,4) 没变,还是bboxes[3,4]是lt,rb;areas是外接大矩形面积;
        instances.denormalize(*img.shape[:2][::-1])  # 没操作

        border = labels.pop("mosaic_border", self.border)  #(0,0)
        self.size = img.shape[1] + border[1] * 2, img.shape[0] + border[0] * 2  # w, h  # (1216,928)
        # M is affine matrix
        # Scale for func:`box_candidates`
        img, M, scale = self.affine_transform(img, border)  # (928,1216,8),[1,0,0],1.0

        bboxes = self.apply_bboxes(instances.bboxes, M)  # 没变

        segments = instances.segments  # (3,100,2)
        keypoints = instances.keypoints  # None
        # Update bboxes if there are segments.
        if len(segments):  # 3,100,2
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if labels is None:
            labels = {}
        img = labels.get("img") if image is None else image  # (900, 1200, 8)
        # shape = img.shape[:2]  # current shape [height, width]  (900,1200)
        new_shape = labels.pop("rect_shape", self.new_shape)  # None (1216,1216)

        top, bottom = 14,14
        left, right = 8,8
        h, w, c = img.shape  # (900,1200,8)
        if c == 3:  # 8
            img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(0, 0, 0))
        else:  # multispectral
            pad_img = np.full((h + top + bottom, w + left + right, c), fill_value=0, dtype=img.dtype)
            pad_img[top : top + h, left : left + w] = img  # (928,1216,8)
            img = pad_img

        if labels.get("ratio_pad"):  # (1,1)
            labels["ratio_pad"] = (labels["ratio_pad"], (left, top))  # for evaluation

        if len(labels):
            labels = self._update_labels(labels, None, left, top)
            labels["img"] = img
...
class Format:
    ...
    Attributes:
        ...
        # return_mask removed - segmentation disabled
        return_keypoint (bool): Whether to return keypoints for pose estimation.
        return_obb (bool): Whether to return oriented bounding boxes.
        mask_ratio (int): Downsample ratio for masks.
        # mask_overlap removed - segmentation disabled
    ...
    Examples:
        >>> formatter = Format(bbox_format="xywh", normalize=True, # return_mask removed - segmentation disabled
    ...
    def __init__(
        ...
        Examples:
            >>> format = Format(bbox_format="xyxy", # return_mask removed - segmentation disabled
            >>> print(format.bbox_format)
            xyxy
        """
        self.bbox_format = bbox_format
        self.normalize = normalize
        self.return_mask = False  # Segmentation disabled
    ...
    def __call__(self, labels):
        ...
        Returns:
            (Dict): A dictionary with formatted data, including:
                ...
                - 'masks': Instance masks tensor (if # return_mask removed - segmentation disabled
        ...
        """
        img = labels.pop("img")  # (928,1216,8)
        h, w = img.shape[:2]  # (928,1216)
        cls = labels.pop("cls") # (22,1)
        instances = labels.pop("instances")  # bbox_areas[22,1] bboxes[22,4]
        instances.convert_bbox(format=self.bbox_format)
        instances.denormalize(w, h)
        nl = len(instances)

        if self.return_mask:  # return_mask removed - segmentation disabled
            if nl:
                masks, instances, cls = self._format_segments(instances, cls, w, h)
                masks = torch.from_numpy(masks)
            else:
                masks = torch.zeros(0)  # No masks for detection/OBB
            labels["masks"] = masks
        labels["img"] = self._format_img(img)  # (8,928,1216)
        labels["cls"] = torch.from_numpy(cls) if nl else torch.zeros(nl)  # (22,1)
        labels["bboxes"] = torch.from_numpy(instances.bboxes) if nl else torch.zeros((nl, 4))
        # if self.return_keypoint:  # False
        #     labels["keypoints"] = torch.from_numpy(instances.keypoints)
        #     if self.normalize:
        #         labels["keypoints"][..., 0] /= w
        #         labels["keypoints"][..., 1] /= h
        if self.return_obb:  # True
            # # 如果label['im_file']是'/data1/users/zhangpeng/datasets/DrMOD_YOLO_NPY/images/train/20231031234632422_00-00512.npy'，则将instances.segments写入txt里
            # if labels['im_file'] == '/data1/users/zhangpeng/datasets/DrMOD_YOLO_NPY/images/train/20231031234632422_00-00512.npy':
            #     with open('instances.segments.txt', 'w') as f:
            #         for segment in instances.segments:
            #             f.write(str(segment) + '\n')
            labels["bboxes"] = (
        ...
        if self.normalize:  # True
            labels["bboxes"][:, [0, 2]] /= w  # 1216
            labels["bboxes"][:, [1, 3]] /= h  # 928
        # Then we can use collate_fn
        if self.batch_idx:  # True
    ...
    def _format_img(self, img):
        ...
        img = img.transpose(2, 0, 1)  # (8,928,1216)
    ...
    def _format_segments(self, instances, cls, w, h):
        ...
        Notes:
            - If self.mask_overlap is True, masks are overlapped
            - If self.mask_overlap is False, masks are separate
            - Masks are downsampled according to self.mask_ratio.
        """
        # Segmentation disabled for OBB - this code should never be executed
        raise NotImplementedError("Segmentation is not supported in OBB mode (use_segments=False)")
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    Examples:
        ...
        >>> hyp = {"mosaic": 1.0, "degrees": 10.0, "translate": 0.2, "scale": 0.9}
